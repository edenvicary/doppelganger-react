...
import { Col, Grid, Image, Row, Modal } from 'react-bootstrap';
...
export default class Home extends Component {
  ...
  getInitial() {
    firebase.auth().onAuthStateChanged(user => {
      this.setState({ imageRef: `images/${user.uid}` });
      if (user) {
        firebase
          .firestore()
          .collection('photos')
  ...
  render() {
    const allImages = this.state.allPhotos.map(photo => {
      if (photo.similarImages) {
        this.doppels = photo.similarImages.map(similarPhoto => {
          const styles = {
        ...
        let loading = () => {
          ...
          for (let i = 0; i < 4; i++) {
            doppels.push(
              <div
              className="main-photo card-1 card loading-card"
              key={i}
              xs={4} >
                <ScaleLoader
                    color="#000"
    ...
    return (
      <div>
        <h1>My Photo Feed</h1>
        <h3>Want to know who Google thinks you look like? Upload a photo by
        clicking the middle button at the bottom to find out...</h3>
